...
class Hotel:
    ...
    def status(self):
        ...
        for room in self.rooms:
            if room.is_taken:
                taken_count.append(room.number)
                self.guests += room.guests
            else:
                free_count.append(room.number)
        output = f'Hotel {self.name} has {self.guests} total guests\n'

        output += f'Free rooms: {", ".join(str(x) for x in free_count)}\n' \
                  f'Taken rooms: {", ".join(str(x) for x in taken_count)}'
